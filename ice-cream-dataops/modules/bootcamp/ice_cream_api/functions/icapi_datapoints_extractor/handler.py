...
def handle(client: CogniteClient = None, data=None):
    ...
    if data:
        ...
        if hours and hours > max_hours:
            logger.warning(
                "%s > %s! The Ice Cream API can't serve more than %s hours of datapoints, setting hours to max",
                hours,
                max_hours,
                max_hours,
            )
    ...
    try:
        for site in sites:
            logger.info("Getting Data Points for %s", site)
            ...
            if to_insert:
                client.time_series.data.insert_multiple(datapoints=to_insert)
                logger.info("%sh of Datapoints took %.2f seconds", hours, default_timer() - big_start)
            else:
                logger.info(
                    "No TimeSeries, for %sh of Datapoints took %.2f seconds", hours, default_timer() - big_start
                )

        report_ext_pipe(client, "success")
    except Exception as e:
        logger.exception("Unhandled error while extracting datapoints")
