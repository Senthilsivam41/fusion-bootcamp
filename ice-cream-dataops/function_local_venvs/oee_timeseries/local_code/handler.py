    ...
    for asset, latest_dps in assets_dps.items():
        ...
        if end:
            ...
            if total_items != len(good_dps) or total_items != len(status_dps) or total_items != len(planned_status_dps):
                ...
                logger.warning(
                    "%s: Unable to retrieve datapoints for all required OEE timeseries (count, good, status, planned_status) "
                    "between %s and %s. Ensure that data is available for the time range specified.",
                    asset,
